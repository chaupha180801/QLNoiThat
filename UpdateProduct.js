import { StyleSheet, Text, View,TouchableOpacity,Image,TextInput,Alert,Platform,
  ...
  export default function AddProduct({route, navigation}) {
    const {id, name, desc, username, fileImage, stock, color, category_id} = route.params;
    
    let [filePath, setFilePath] = useState(JSON.parse(fileImage));
    let [dropdown, setDropdown] = useState(category_id);
    let [dropdown_color, setDropdownColor] = useState(color);
    let [items, setItems] = useState([]);
    let [product_name, setName] = useState(name);
    let [inputDesc, setDesc] = useState(desc);
    let [inputStock, setStock] = useState(String(stock));
    const reset = () => {
      setDesc('');
      setName('');
      setStock('');
    ...
    const update_product = () => {
      ...
      if (!inputStock) {
        ToastAndroid.showWithGravity('Vui lòng nhập số lượng sản phẩm',
        ToastAndroid.SHORT,
        ToastAndroid.CENTER
        );
        return;
      ...
      db.transaction(function (tx) {
          tx.executeSql(
            'UPDATE SANPHAM SET TENSP = ? , MOTA = ?, HINHANH = ?, SOLUONG = ?, MAU = ?, MADANHMUC = ? WHERE MASP = ?',
            [product_name, inputDesc, JSON.stringify(filePath), inputStock, dropdown_color, dropdown, id],      
            (tx, results) => {
              console.log('Results executeSql update into sp: ', results.rowsAffected);
    ...
    return (
      <View>
          ...
          <View styles={styles.header}>
            ...
            <Text style={styles.text_header}>Thông tin sản phẩm</Text>   
          </View>
          ...
          <TouchableOpacity style={styles.button1} onPress={update_product}>
